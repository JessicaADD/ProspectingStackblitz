  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", gap: 80, fontFamily: font, background: "linear-gradient(135deg, #0F172A 0%, #1E293B 50%, #334155 100%)", position: "relative", overflow: "hidden", padding: "40px 60px" }}>
      ...
      <div style={{ maxWidth: 460, position: "relative", zIndex: 1 }}>
        ...
        <div style={{ marginTop: 60, color: "rgba(255,255,255,0.2)", fontSize: 12 }}>© {new Date().getFullYear()} Auto Dealers Digital</div>
      </div>
...
function UploadModal({ onClose, onDone }) {
  ...
  const handleFile = async (f) => { if (!f) return; try { setParsing(true); setFn(f.name); const p = await parseCSV(f); setUrls(p); setEr(p.length === 0 ? "No websites found — check your CSV has one domain per line (e.g. dealer.com or https://dealer.com)" : ""); } catch { setEr("Read failed"); } finally { setParsing(false); } };
      ...
      <table style={{ width: "100%", borderCollapse: "collapse" }}>
        ...
        <tbody>{lists.length === 0 && <tr><td colSpan={6}><Empty text="No lists yet — click 'New list' to get started" /></td></tr>}
          {lists.map((l) => <tr key={l.id} style={{ borderBottom: `1px solid #F1F5F9`, cursor: "pointer" }} onClick={() => { setSel(l); setView("lists"); }} onMouseEnter={(e) => (e.currentTarget.style.background = "#F8FAFC")} onMouseLeave={(e) => (e.currentTarget.style.background = "transparent")}>
            ...
            <td style={{ padding: "16px" }}>{ (() => { const wsIds = new Set((l.websites || []).map(w => w.id)); const total = wsIds.size; if (!total) return <span style={{ fontSize: 13, color: P.text3 }}>—</span>; const visited = (pr || []).filter(p => wsIds.has(p.website_id) && p.visited).length; return <Progress pct={Math.round((visited / total) * 100)} />; })() }</td>
...
function RepListView({ list, assignment, onBack }) {
  ...
  return <div>
    ...
    <Err msg={er} />
    <div style={{ background: P.blueBg, border: `1px solid ${P.blueBorder}`, borderRadius: 8, padding: "10px 16px", fontSize: 13, color: P.blue, marginBottom: 16, display: "flex", alignItems: "center", gap: 8 }}><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={P.blue} strokeWidth="2"><circle cx="12" cy="12" r="10"/><path d="M12 16v-4M12 8h.01"/></svg>Click a website link to open it — <strong>Visited</strong> is marked automatically. Use <strong>Claimed</strong> if you spoke to someone, and <strong>Not open</strong> if the business is closed.</div>
...
function RepApp() {
  ...
  return <><h1 style={{ fontSize: 28, fontWeight: 700, marginBottom: 4, color: P.text, letterSpacing: "-0.02em" }}>My lists</h1><p style={{ fontSize: 15, color: P.text3, marginBottom: 24 }}>Your assigned prospecting lists</p>{ld ? <Loading /> : al.length === 0 ? <div style={{ ...card, padding: 60, textAlign: "center" }}><div style={{ fontSize: 40, marginBottom: 16 }}>📋</div><div style={{ fontSize: 18, fontWeight: 600, color: P.text, marginBottom: 8 }}>No lists assigned yet</div><div style={{ fontSize: 14, color: P.text3 }}>Your manager will assign you a prospecting list soon.</div></div> : <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: 16 }}>{al.map((a) => { const l = a.lists; const total = Math.max(0, (a.end_position || 0) - (a.start_position || 0) + 1); const visited = prog.filter(p => p.visited).length; const pct = total > 0 ? Math.round((visited / total) * 100) : 0; return <div key={`${a.list_id}_${a.rep_id}`} onClick={() => setSel(a)} style={{ ...card, padding: 24, cursor: "pointer", transition: "all 0.15s" }} onMouseEnter={(e) => { e.currentTarget.style.borderColor = P.blue; e.currentTarget.style.boxShadow = P.shadowMd; }} onMouseLeave={(e) => { e.currentTarget.style.borderColor = P.border; e.currentTarget.style.boxShadow = P.shadow; }}>
    ...
    <div style={{ fontSize: 14, color: P.text3, marginBottom: 12 }}>{total > 0 ? `${total} websites (#${a.start_position}–#${a.end_position})` : "All websites"}</div>
