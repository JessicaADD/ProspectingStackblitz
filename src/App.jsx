...
function parseCSV(file) { return new Promise((res, rej) => { const r = new FileReader(); r.onerror = () => rej(new Error("Read failed")); r.onload = (e) => { const l = e.target.result.split(/[\n\r]+/).flatMap((row) => row.split(/[,;\t]+/).map((x) => { let s = x.trim().replace(/^["']+|["']+$/g, ""); s = s.replace(/^https?:\/\//i, "").replace(/^www\./i, "").replace(/\/$/, ""); return s.toLowerCase(); })).filter((x) => x && x.includes(".") && x.length > 3 && !x.includes(" ") && !/^(website|url|domain|name|header|address|http)/i.test(x)); res([...new Set(l)]); }; r.readAsText(file); }); }
const fmt = (d) => { if (!d) return "â"; const dt = new Date(d); return dt.toLocaleDateString("en-US", { month: "short", day: "numeric" }) + ", " + dt.toLocaleTimeString("en-US", { hour: "numeric", minute: "2-digit" }); };
...
function Empty({ icon = "ð­", text }) { return <div style={{ padding: 60, textAlign: "center", color: P.text3, fontSize: 14 }}><div style={{ fontSize: 32, marginBottom: 12 }}>{icon}</div>{text}</div>; }
    ...
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", gap: 80, fontFamily: font, background: "linear-gradient(135deg, #0F172A 0%, #1E293B 50%, #334155 100%)", position: "relative", overflow: "hidden", padding: "40px 60px" }}>
      ...
      <div style={{ maxWidth: 460, position: "relative", zIndex: 1 }}>
        ...
        <div style={{ display: "flex", gap: 16 }}>
          {[["â¡", "Fast", "Find and track prospects at scale"], ["ð¯", "Focused", "Reps see only their assigned work"], ["ð", "Measured", "Real-time progress tracking"]].map(([icon, title, desc]) => (
        ...
        <div style={{ marginTop: 60, color: "rgba(255,255,255,0.2)", fontSize: 12 }}>Â© {new Date().getFullYear()} Auto Dealers Digital</div>
      </div>
      <div style={{ position: "relative", zIndex: 1, flexShrink: 0 }}>
        <div style={{ width: 360, background: "rgba(255,255,255,0.97)", borderRadius: 16, padding: "40px 32px", boxShadow: "0 25px 60px rgba(0,0,0,0.3)" }}>
          ...
          <input value={pw} onChange={(e) => setPw(e.target.value)} onKeyDown={(e) => e.key === "Enter" && go()} placeholder="â¢â¢â¢â¢â¢â¢â¢â¢" type="password" style={{ ...inp, marginBottom: 24 }} />
          ...
          <button onClick={go} disabled={loading} style={{ width: "100%", padding: 12, background: P.blue, border: "none", borderRadius: 10, color: "#fff", fontSize: 16, fontWeight: 600, cursor: loading ? "not-allowed" : "pointer", opacity: loading ? 0.7 : 1, display: "flex", alignItems: "center", justifyContent: "center", gap: 8, fontFamily: font, transition: "opacity 0.15s" }}>
            {loading && <Spinner />} {loading ? "Signing in..." : "Sign in â"}
          </button>
        </div>
      </div>
    </div>
...
function Modal({ children, onClose, width = 480 }) {
  return <div style={{ position: "fixed", inset: 0, background: "rgba(15,23,42,0.5)", backdropFilter: "blur(4px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 200 }} onClick={(e) => e.target === e.currentTarget && onClose()}><div style={{ background: P.card, borderRadius: 16, padding: 32, width, maxWidth: "95vw", maxHeight: "90vh", overflow: "auto", boxShadow: P.shadowLg, position: "relative" }}><button onClick={onClose} style={{ position: "absolute", top: 16, right: 16, width: 30, height: 30, borderRadius: "50%", border: `1px solid ${P.border}`, background: P.bg, color: P.text3, cursor: "pointer", fontSize: 16, display: "flex", alignItems: "center", justifyContent: "center", lineHeight: 1, fontFamily: font }}>â</button>{children}</div></div>;
}

function UploadModal({ onClose, onDone }) {
  const { session } = useApp();
  const [name, setName] = useState(""); const [prov, setProv] = useState(""); const [state, setState] = useState("");
  const [urls, setUrls] = useState([]); const [fn, setFn] = useState(""); const [ld, setLd] = useState(false); const [parsing, setParsing] = useState(false); const [er, setEr] = useState(""); const [drag, setDrag] = useState(false);
  const handleFile = async (f) => { if (!f) return; try { setParsing(true); setFn(f.name); const p = await parseCSV(f); setUrls(p); setEr(p.length === 0 ? "No websites found â check your CSV has one domain per line (e.g. dealer.com or https://dealer.com)" : ""); } catch { setEr("Read failed"); } finally { setParsing(false); } };
  const hf = async (e) => { handleFile(e.target.files?.[0]); };
  ...
  return <Modal onClose={onClose}><h2 style={{ margin: "0 0 4px", fontSize: 20, fontWeight: 700, color: P.text }}>Upload new list</h2><p style={{ color: P.text3, fontSize: 14, margin: "0 0 24px" }}>Add a new prospecting list to your workspace</p>
    ...
    <div style={{ marginBottom: 20 }}><label style={{ display: "block", fontSize: 14, color: P.text2, marginBottom: 6, fontWeight: 500 }}>CSV file <span style={{ color: P.red }}>*</span></label><label onDragOver={(e) => { e.preventDefault(); setDrag(true); }} onDragLeave={() => setDrag(false)} onDrop={onDrop} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 8, padding: 28, background: drag ? P.blueBg : P.bg, border: `2px dashed ${drag ? P.blue : urls.length > 0 ? P.green : P.border}`, borderRadius: 12, textAlign: "center", cursor: "pointer", transition: "all 0.15s" }}><input type="file" accept=".csv,.txt" onChange={hf} style={{ display: "none" }} />{parsing ? <><Spinner size={24} /><div style={{ color: P.text3, fontSize: 14 }}>Reading file...</div></> : <><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke={urls.length > 0 ? P.green : P.text3} strokeWidth="2"><path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4M17 8l-5-5-5 5M12 3v12"/></svg><div style={{ color: P.text2, fontSize: 14 }}>{fn || "Click to upload or drag and drop"}</div><div style={{ color: P.text3, fontSize: 12 }}>CSV or TXT Â· one domain per row</div>{urls.length > 0 && <div style={{ color: P.green, fontSize: 14, fontWeight: 600 }}>â {urls.length} websites found</div>}</>}</label></div>
    <Err msg={er} /><div style={{ display: "flex", gap: 10, justifyContent: "flex-end", paddingTop: 8 }}><Btn onClick={onClose} variant="ghost">Cancel</Btn><Btn onClick={go} disabled={ld || parsing}>{ld ? "Uploading..." : "Upload list"}</Btn></div></Modal>;
}
...
function AssignModal({ list, reps, assignments, wc, token, onClose, onDone }) {
  ...
  return <Modal onClose={onClose} width={560}><h2 style={{ margin: "0 0 4px", fontSize: 20, fontWeight: 700, color: P.text }}>Assign reps</h2><p style={{ color: P.text3, fontSize: 14, margin: "0 0 20px" }}>{list.name} Â· {wc} websites</p>
    <div style={{ display: "flex", gap: 12, marginBottom: 20 }}><StatCard label="Total" value={wc} icon="ð" /><StatCard label="Assigned" value={total} color={P.green} icon="â" /><StatCard label="Open" value={Math.max(0, wc - total)} color={wc - total > 0 ? P.red : P.text3} icon="ð" /></div>
...
function ReassignModal({ list, reps, assignments, websites, progress, token, onClose, onDone }) {
  ...
  return <Modal onClose={onClose} width={560}><h2 style={{ margin: "0 0 4px", fontSize: 20, fontWeight: 700, color: P.text }}>Reassign websites</h2><p style={{ color: P.text3, fontSize: 14, margin: "0 0 20px" }}>Move websites from one rep to another</p>
    <div style={{ display: "flex", gap: 12, marginBottom: 20 }}><div style={{ flex: 1 }}><label style={{ fontSize: 14, color: P.text2, marginBottom: 6, display: "block", fontWeight: 500 }}>From rep</label><select value={fromRep} onChange={(e) => { setFromRep(e.target.value); setSelPos(new Set()); setToRep(""); }} style={inp}><option value="">Select...</option>{assignments.map((a) => <option key={a.rep_id} value={a.rep_id}>{a.profiles?.full_name} (#{a.start_position}â#{a.end_position})</option>)}</select></div><div style={{ display: "flex", alignItems: "flex-end", paddingBottom: 10, fontSize: 20, color: P.text3 }}>â</div><div style={{ flex: 1 }}><label style={{ fontSize: 14, color: P.text2, marginBottom: 6, display: "block", fontWeight: 500 }}>To rep</label><select value={toRep} onChange={(e) => setToRep(e.target.value)} style={inp}><option value="">Select...</option>{toOpts.map((r) => { const a = assignments.find((a) => a.rep_id === r.id); return <option key={r.id} value={r.id}>{r.full_name}{a ? ` (#${a.start_position}â#${a.end_position})` : " (new)"}</option>; })}</select></div></div>
    {fromRep && <><div style={{ marginBottom: 16 }}><div style={{ fontSize: 14, color: P.text2, marginBottom: 8, fontWeight: 500 }}>What to move</div><div style={{ display: "flex", gap: 8 }}>{[["unvisited", `Unvisited (${unvis.length})`], ["all", `All (${fromWs.length})`], ["selected", "Manual"]].map(([v, l]) => <button key={v} onClick={() => setMode(v)} style={{ padding: "8px 16px", borderRadius: 8, border: `1.5px solid ${mode === v ? P.blue : P.border}`, background: mode === v ? P.blueBg : P.card, color: mode === v ? P.blue : P.text2, cursor: "pointer", fontSize: 14, fontWeight: 500, fontFamily: font }}>{l}</button>)}</div></div>
      ...
      {mc > 0 && <div style={{ background: P.amberBg, border: `1px solid ${P.amberBorder}`, borderRadius: 8, padding: "12px 16px", fontSize: 14, color: P.amber, marginBottom: 16, fontWeight: 500 }}>â ï¸ {mc} website{mc !== 1 ? "s" : ""} will be moved{toRep ? ` to ${reps.find((r) => r.id === toRep)?.full_name}` : ""}</div>}</>}
...
function EditListModal({ list, token, onClose, onDone }) {
  ...
  const [urls, setUrls] = useState([]); const [fn, setFn] = useState(""); const [repl, setRepl] = useState(false); const [confirmed, setConfirmed] = useState(false); const [ld, setLd] = useState(false); const [er, setEr] = useState(""); const [drag, setDrag] = useState(false);
  const handleFile = async (f) => { if (!f) return; try { setFn(f.name); const p = await parseCSV(f); setUrls(p); } catch { setEr("Read failed"); } };
  const hf = (e) => handleFile(e.target.files?.[0]);
  const onDrop = (e) => { e.preventDefault(); setDrag(false); handleFile(e.dataTransfer.files?.[0]); };
  const go = async () => { if (!name.trim()) return setEr("Name required"); if (repl && urls.length && !confirmed) return setEr("Please confirm you want to replace the CSV"); setLd(true); setEr(""); try { await sb(`/rest/v1/lists?id=eq.${list.id}`, { method: "PATCH", token, body: { name: name.trim(), provider: prov.trim() || null, state: state.trim() || null } }); if (repl && urls.length && confirmed) { await sb(`/rest/v1/websites?list_id=eq.${list.id}`, { method: "DELETE", token }); await sb(`/rest/v1/assignments?list_id=eq.${list.id}`, { method: "DELETE", token }); const rows = urls.map((u, i) => ({ list_id: list.id, url: u, position: i + 1 })); for (let i = 0; i < rows.length; i += 100) await sb("/rest/v1/websites", { method: "POST", token, body: rows.slice(i, i + 100) }); } onDone(); } catch (e) { setEr(e.message); } setLd(false); };
  return <Modal onClose={onClose}><h2 style={{ margin: "0 0 4px", fontSize: 20, fontWeight: 700, color: P.text }}>Edit list</h2><p style={{ color: P.text3, fontSize: 14, margin: "0 0 24px" }}>Update list details or replace CSV</p>
    ...
    {!repl ? <div style={{ marginBottom: 20 }}><Btn onClick={() => setRepl(true)} variant="danger" small>Replace CSV file</Btn><p style={{ fontSize: 13, color: P.text3, marginTop: 6 }}>This will permanently delete all websites and assignments</p></div> : <div style={{ marginBottom: 20 }}><label onDragOver={(e) => { e.preventDefault(); setDrag(true); }} onDragLeave={() => setDrag(false)} onDrop={onDrop} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6, padding: 24, background: drag ? P.redBg : P.redBg, border: `2px dashed ${drag ? P.red : P.redBorder}`, borderRadius: 12, textAlign: "center", cursor: "pointer" }}><input type="file" accept=".csv,.txt" onChange={hf} style={{ display: "none" }} /><div style={{ color: P.red, fontSize: 14, fontWeight: 500 }}>{fn || "Click or drag to select new CSV"}</div>{urls.length > 0 && <div style={{ color: P.green, fontSize: 13, fontWeight: 600 }}>â {urls.length} websites found</div>}</label>{urls.length > 0 && <div onClick={() => setConfirmed(!confirmed)} style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 12, cursor: "pointer", padding: "10px 14px", background: confirmed ? "#FEF2F2" : P.bg, border: `1px solid ${confirmed ? P.redBorder : P.border}`, borderRadius: 8 }}><Check checked={confirmed} color={P.red} /><span style={{ fontSize: 13, color: P.red, fontWeight: 500 }}>I understand this will delete all existing websites and assignments</span></div>}<button onClick={() => { setRepl(false); setUrls([]); setConfirmed(false); }} style={{ background: "none", border: "none", color: P.text3, cursor: "pointer", fontSize: 13, marginTop: 8, fontFamily: font, display: "block" }}>Cancel replacement</button></div>}
...
// âââ LIST DETAIL ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
    ...
    <div style={{ ...card, padding: 24, marginBottom: 24 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          {asgn.length >= 1 && <Btn onClick={() => setShowReassign(true)} variant="warning" small>â Reassign</Btn>}
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 12 }}>{asgn.map((a, i) => { const s = rs(a); const pct = s.total > 0 ? (s.visited / s.total) * 100 : 0; return <div key={a.rep_id} style={{ background: P.greenBg, border: `1px solid ${P.greenBorder}`, borderRadius: 10, padding: "16px 20px", minWidth: 200, flex: "1 1 200px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}><Avatar name={a.profiles?.full_name} idx={i} size={32} /><div><div style={{ fontWeight: 600, color: P.green, fontSize: 15 }}>{a.profiles?.full_name}</div><div style={{ fontSize: 13, color: P.text3 }}>#{a.start_position}â#{a.end_position} ({s.total} sites)</div></div></div>
        <Progress pct={pct} />
        <div style={{ display: "flex", gap: 16, fontSize: 13, color: P.text2, marginTop: 10 }}><span>ð {s.visited} visited</span><span>â {s.claimed} claimed</span><span>ð« {s.notOpen} not open</span></div>
      </div>; })}</div>}
    </div>
    ...
    <div style={card}>
      ...
      <div style={{ overflow: "auto" }}>
        <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 600 }}>
          <thead><tr style={{ background: P.bg }}>{["#", "Website", "Assigned to", "Last activity", ...asgn.map((a) => a.profiles?.full_name)].map((h, i) => <th key={i} style={{ padding: "12px 16px", textAlign: i > 2 ? "center" : "left", fontSize: 12, fontWeight: 600, color: P.text3, textTransform: "uppercase", letterSpacing: "0.06em", borderBottom: `1px solid ${P.border}` }}>{h}</th>)}</tr>{asgn.length > 0 && <tr style={{ background: "#FFFBEB" }}><td colSpan={4} />{asgn.map((a) => <td key={a.rep_id} style={{ padding: "4px 16px", textAlign: "center", fontSize: 11, color: P.text3, borderBottom: `1px solid ${P.border}`, whiteSpace: "nowrap" }}>ð visited &nbsp;â claimed &nbsp;ð« not open</td>)}</tr>}</thead>
          <tbody>{ws.length === 0 && <tr><td colSpan={4 + asgn.length}><Empty text="No websites" /></td></tr>}
            {ws.map((w) => { const as = asgn.find((a) => w.position >= (a.start_position || 1) && w.position <= (a.end_position || ws.length)); const wp = prog.filter((p) => p.website_id === w.id); const la = wp.reduce((l, p) => { const d = p.updated_at; return d && (!l || new Date(d) > new Date(l)) ? d : l; }, null);
              return <tr key={w.id} style={{ borderBottom: `1px solid #F1F5F9` }}>
                ...
                <td style={{ padding: "12px 16px" }}>{as ? <Badge color={P.green} bg={P.greenBg}>{as.profiles?.full_name}</Badge> : <span style={{ color: P.text3 }}>â</span>}</td>
                <td style={{ padding: "12px 16px", fontSize: 13, color: P.text3 }}>{fmt(la)}</td>
                {asgn.map((a) => { if (w.position < (a.start_position || 1) || w.position > (a.end_position || ws.length)) return <td key={a.rep_id} style={{ padding: "12px 16px", textAlign: "center", color: "#E2E8F0" }}>â</td>; const p = prog.find((p) => p.website_id === w.id && p.rep_id === a.rep_id); return <td key={a.rep_id} style={{ padding: "12px 16px", textAlign: "center" }}><div style={{ display: "flex", justifyContent: "center", gap: 8, fontSize: 14 }}><span>{p?.visited ? "ð" : "Â·"}</span><span>{p?.claimed ? "â" : "Â·"}</span><span>{p?.not_open ? "ð«" : "Â·"}</span></div></td>; })}
              </tr>; })}</tbody>
        </table>
      </div>
    </div>
...
// âââ MANAGER VIEWS ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
function AlertsView({ alerts }) { const da = (d) => Math.floor((new Date() - new Date(d)) / 86400000); return <div><h1 style={{ fontSize: 28, fontWeight: 700, marginBottom: 4, color: P.text, letterSpacing: "-0.02em" }}>Alerts</h1><p style={{ fontSize: 15, color: P.text3, marginBottom: 24 }}>Reps with no activity after 7+ days</p>{alerts.length === 0 ? <div style={card}><Empty icon="ð" text="No alerts â all reps are active" /></div> : <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>{alerts.map((a) => <div key={a.id} style={{ ...card, padding: "18px 24px", display: "flex", gap: 16, alignItems: "center" }}><div style={{ width: 42, height: 42, borderRadius: 10, background: P.amberBg, border: `1px solid ${P.amberBorder}`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18, flexShrink: 0 }}>â ï¸</div><div style={{ flex: 1 }}><div style={{ fontSize: 15, color: P.text, lineHeight: 1.5 }}><span style={{ fontWeight: 600 }}>{a.repName}</span> hasn't opened any website from <span style={{ fontWeight: 600 }}>{a.listName}</span></div><div style={{ fontSize: 13, color: P.text3, marginTop: 2 }}>Assigned {da(a.assignedAt)} days ago</div></div></div>)}</div>}</div>; }
...
// âââ MANAGER APP ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
function ManagerApp({ view, setView, setAlerts }) {
  ...
  const [ld, setLd] = useState(true); const [er, setEr] = useState(""); const [alerts, setLA] = useState([]); const [progress, setProgress] = useState([]);
  const load = useCallback(async () => { setLd(true); setEr(""); try { const [ls, rs, pr] = await Promise.all([sb("/rest/v1/lists?select=*,assignments(rep_id,start_position,end_position,created_at,profiles(full_name)),websites(id)&order=created_at.desc", { token: tk }), sb("/rest/v1/profiles?role=eq.rep&app=eq.prospecting&select=*&order=full_name.asc", { token: tk }), sb("/rest/v1/progress?select=*", { token: tk })]); setLists(ls || []); setReps(rs || []); setProgress(pr || []); const repIds = new Set((rs || []).map(r => r.id)); const now = new Date(); const al = []; for (const l of (ls || [])) { if (!l.assignments) continue; for (const a of l.assignments) { if (!repIds.has(a.rep_id)) continue; if (!a.created_at || Math.floor((now - new Date(a.created_at)) / 86400000) < 7) continue; if (!(pr || []).some((p) => p.rep_id === a.rep_id && p.visited)) al.push({ id: `${l.id}_${a.rep_id}`, listName: l.name, repName: a.profiles?.full_name || "?", assignedAt: a.created_at }); } } setLA(al); setAlerts(al); } catch (e) { setEr(e.message); } setLd(false); }, [tk, setAlerts]);
  useEffect(() => { load(); }, [load]);
  ...
  return <>
    ...
    {ld ? <Loading /> : <div style={card}><div style={{ overflow: "auto" }}>
      <table style={{ width: "100%", borderCollapse: "collapse" }}>
        ...
        <tbody>{lists.length === 0 && <tr><td colSpan={6}><Empty text="No lists yet â click 'New list' to get started" /></td></tr>}
          {lists.map((l) => <tr key={l.id} style={{ borderBottom: `1px solid #F1F5F9`, cursor: "pointer" }} onClick={() => { setSel(l); setView("lists"); }} onMouseEnter={(e) => (e.currentTarget.style.background = "#F8FAFC")} onMouseLeave={(e) => (e.currentTarget.style.background = "transparent")}>
            ...
            <td style={{ padding: "16px" }}>{l.provider ? <Badge>{l.provider}</Badge> : <span style={{ color: P.text3 }}>â</span>}</td>
            ...
            <td style={{ padding: "16px" }}>{ (() => { const wsIds = new Set((l.websites || []).map(w => w.id)); const total = wsIds.size; if (!total) return <span style={{ fontSize: 13, color: P.text3 }}>â</span>; const visited = progress.filter(p => wsIds.has(p.website_id) && p.visited).length; return <Progress pct={Math.round((visited / total) * 100)} />; })() }</td>
...
// âââ REP VIEWS ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
function RepListView({ list, assignment, onBack }) {
  ...
  return <div>
    ...
    <p style={{ color: P.text3, fontSize: 15, margin: "0 0 24px" }}>Your websites: #{sp}â#{ep}</p>
    <div style={{ display: "flex", gap: 12, marginBottom: 24, flexWrap: "wrap" }}><StatCard label="Assigned" value={st.total} icon="ð" /><StatCard label="Visited" value={st.visited} color={P.blue} icon="ð" /><StatCard label="Claimed" value={st.claimed} color={P.green} icon="â" /><StatCard label="Not open" value={st.notOpen} color={P.red} icon="ð«" /></div>
    <Err msg={er} />
    <div style={{ background: P.blueBg, border: `1px solid ${P.blueBorder}`, borderRadius: 8, padding: "10px 16px", fontSize: 13, color: P.blue, marginBottom: 16, display: "flex", alignItems: "center", gap: 8 }}><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={P.blue} strokeWidth="2"><circle cx="12" cy="12" r="10"/><path d="M12 16v-4M12 8h.01"/></svg>Click a website link to open it â <strong>Visited</strong> is marked automatically. Use <strong>Claimed</strong> if you spoke to someone, and <strong>Not open</strong> if the business is closed.</div>
...
function RepApp() {
  const { session } = useApp();
  const [al, setAl] = useState([]); const [prog, setProg] = useState([]); const [sel, setSel] = useState(null); const [ld, setLd] = useState(true);
  useEffect(() => { Promise.all([sb(`/rest/v1/assignments?rep_id=eq.${session.user.id}&select=*,lists(*)`, { token: session.token }), sb(`/rest/v1/progress?rep_id=eq.${session.user.id}&select=*`, { token: session.token })]).then(([d, p]) => { setAl((d || []).filter((a) => a.lists)); setProg(p || []); setLd(false); }).catch(() => setLd(false)); }, [session.user.id, session.token]);
  if (sel) return <RepListView list={sel.lists} assignment={sel} onBack={() => setSel(null)} />;
  return <><h1 style={{ fontSize: 28, fontWeight: 700, marginBottom: 4, color: P.text, letterSpacing: "-0.02em" }}>My lists</h1><p style={{ fontSize: 15, color: P.text3, marginBottom: 24 }}>Your assigned prospecting lists</p>{ld ? <Loading /> : al.length === 0 ? <div style={{ ...card, padding: 60, textAlign: "center" }}><div style={{ fontSize: 40, marginBottom: 16 }}>ð</div><div style={{ fontSize: 18, fontWeight: 600, color: P.text, marginBottom: 8 }}>No lists assigned yet</div><div style={{ fontSize: 14, color: P.text3 }}>Your manager will assign you a prospecting list soon.</div></div> : <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: 16 }}>{al.map((a) => { const l = a.lists; const total = Math.max(0, (a.end_position || 0) - (a.start_position || 0) + 1); const visited = prog.filter(p => p.visited).length; const pct = total > 0 ? Math.round((visited / total) * 100) : 0; return <div key={`${a.list_id}_${a.rep_id}`} onClick={() => setSel(a)} style={{ ...card, padding: 24, cursor: "pointer", transition: "all 0.15s" }} onMouseEnter={(e) => { e.currentTarget.style.borderColor = P.blue; e.currentTarget.style.boxShadow = P.shadowMd; }} onMouseLeave={(e) => { e.currentTarget.style.borderColor = P.border; e.currentTarget.style.boxShadow = P.shadow; }}>
    ...
    <div style={{ fontSize: 14, color: P.text3, marginBottom: 12 }}>{total > 0 ? `${total} websites (#${a.start_position}â#${a.end_position})` : "All websites"}</div>
    {total > 0 && <div style={{ marginBottom: 12 }}><Progress pct={pct} /><div style={{ fontSize: 12, color: P.text3, marginTop: 4 }}>{visited} of {total} visited</div></div>}
